...
function makeCities(){
  ...
  function cities(){
    return test.map(function(item) {
     return {
       value: item.Description,
       lable: item.Ref,
       valueRu: item.DescriptionRu
     }
   })
 };
  cities.set =  function(value){
    test = value;
  }
  return cities;
}
...
$(function(){
    ...
    success: function(data){
      Cities.set(data.data.map(function(citi){
...
$(document).on('click','#button-shipping-method' , function() {
  var selector = $('#np,#pu').find('input[name=shipping_method]:checked').parents()[1].id;
    $.ajax({
